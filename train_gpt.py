...
class Hyperparameters:
    ...
    ttt_lora_rank = int(os.environ.get("TTT_LORA_RANK", 4))  # LoRA rank for TTT adapters

    # Quantization.
    fp16_embed = bool(int(os.environ.get("FP16_EMBED", "1")))  # keep embeddings in FP16
    quant_bits = int(os.environ.get("QUANT_BITS", 4))  # quantization bit width for attention (6 or 8)
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 11))  # unique layers
...
def get_orthogonal_matrix(N: int, seed: int, device: torch.device) -> Tensor:
    ...
    signs = torch.randint(0, 2, (N,), generator=rng, dtype=torch.float32, device='cpu') * 2 - 1
    return signs.to(device).view(-1, 1) * D
...
def quantize_float_tensor(t: Tensor, bits: int = 4, search_clip: bool = False, turboquant: bool = False, name: str = "") -> tuple[Tensor, Tensor]:
    qmax = (1 << (bits - 1)) - 1
    t32 = t.float()

    if turboquant and t32.ndim == 2:
        in_dim = t32.size(1)
        seed = int(hashlib.sha256(name.encode('utf-8')).hexdigest(), 16) % (2**31)
        R = get_orthogonal_matrix(in_dim, seed, t32.device)
        
        # TurboQuant Stage 1: PolarQuant Rotation
        t32_rot = t32 @ R.T
        
        # Matrix is beautifully Normal. Use standard scalar quantizer.
        best_q, best_scale, best_mse = None, None, float("inf")
        candidates = [0.99, 0.999, 0.9995, 0.9999, 1.0] if search_clip else [0.999]
        for pct in candidates:
            clip_abs = float(torch.quantile(t32_rot.abs().flatten(), pct).item()) if t32_rot.numel() else 0.0
            scale = torch.tensor(clip_abs / qmax if clip_abs > 0 else 1.0, dtype=torch.float32, device=t32.device)
            q = torch.clamp(torch.round(torch.clamp(t32_rot, -clip_abs, clip_abs) / scale), -qmax, qmax).to(torch.int8)
            recon = q.float() * scale
            mse = ((t32_rot - recon) ** 2).mean().item()
            if mse < best_mse:
                best_mse = mse
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], fp16_embed: bool = False, quant_bits: int = 8, quant_bits_mlp: int = 0, search_clip: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        is_turbo = (t.ndim == 2 and "tok_emb" not in name and "bigram_hash" not in name)
        q, s = quantize_float_tensor(t, bits=bits, search_clip=search_clip, turboquant=is_turbo, name=name)
        
        if is_turbo:
            qmeta[name] = {"scheme": "turboquant", "axis": None}
        elif s.ndim > 0:
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        s = obj["scales"][name]
        is_turbo = qmeta.get(name, {}).get("scheme") == "turboquant"
        
        if is_turbo:
            scale = float(s.item())
            w_rot_approx = q.float() * scale
            in_dim = w_rot_approx.size(1)
            seed = int(hashlib.sha256(name.encode('utf-8')).hexdigest(), 16) % (2**31)
            R = get_orthogonal_matrix(in_dim, seed, w_rot_approx.device)
            out[name] = (w_rot_approx @ R).to(dtype=dtype).contiguous()
...
class FakeQuantizeCAGE(torch.autograd.Function):
    """CAGE: Curvature-aware QAT gradient estimator."""
    @staticmethod
    def forward(ctx, w: Tensor, bits: int, v_hat: Tensor | None = None) -> Tensor:
        qmax = (1 << (bits - 1)) - 1
        if w.ndim == 2:
        ...
        qw = (torch.clamp(torch.round(w / scale), -qmax, qmax) * scale).to(w.dtype)
        ctx.save_for_backward(v_hat)
        return qw

    @staticmethod
    def backward(ctx, grad: Tensor) -> tuple[Tensor, None, None]:
        v_hat, = ctx.saved_tensors
        if v_hat is not None:
            # Scale gradient by inverse curvature
            grad = grad / (v_hat.sqrt() + 1e-8)
        return grad, None, None


class CastedLinear(nn.Linear):
    _qat_bits: int = 0
    _lora_A: Tensor | None = None
    _lora_B: Tensor | None = None

    _v_hat: Tensor | None = None

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self._qat_bits > 0 and self.weight.numel() > 65536:
            w = FakeQuantizeCAGE.apply(w, self._qat_bits, self._v_hat)
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        out = F.linear(x, w, bias)
        if self._lora_A is not None and self._lora_B is not None:
            lx = x.to(dtype=self._lora_A.dtype)
            out = out + F.linear(F.linear(lx, self._lora_A), self._lora_B).to(dtype=out.dtype)
        return out
...
class HymbaAttention(nn.Module):
    ...
    def __init__(
        ...
        self.v_gate = CastedLinear(dim, kv_dim, bias=False)  # GLU on Values
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        v_out = v_out * torch.sigmoid(self.v_gate(x))  # GLU on Values
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        v_raw = self.c_v(x) * torch.sigmoid(self.v_gate(x))  # GLU on Values
...
def main() -> None:
    ...
    if hasattr(base_model, 'skip_weights') and base_model.skip_weights.numel() > 0:
        adam_params.append(base_model.skip_weights)
    if base_model.smeargate is not None:
        adam_params.append(base_model.smeargate.gate)
    if base_model.bigram_hash is not None:
        adam_params.append(base_model.bigram_hash.table.weight)
        # bigram_hash.proj.weight is a 2D matrix
        mousse_params.append(base_model.bigram_hash.proj.weight)
    ...
    optimizer_adam = torch.optim.AdamW(
        adam_params,
        lr=args.scalar_lr, # Using scalar_lr for AdamW group
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        weight_decay=0.04, # Using a fixed weight_decay for AdamW group
    )
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_mousse, optimizer_adam]
    ...
    while True:
        ...
        if args.qat_start_frac > 0 and max_wallclock_ms:
            ...
            # Pre-Flight Check 3: CAGE Initialization Shock. Ensure LR scale < 0.15
            if qat_active and scale > 0.15:
                qat_active = False
        ...
        # Inject v_hat into CastedLinear layers for CAGE QAT
        if args.qat_start_frac > 0 and max_wallclock_ms and elapsed_ms / max_wallclock_ms >= args.qat_start_frac:
            for m in base_model.modules():
                if isinstance(m, CastedLinear) and m.weight in optimizer_mousse.state:
                    state = optimizer_mousse.state[m.weight]
                    m._v_hat = state.get("v_hat")
        ...
        for micro_step in range(grad_accum_steps):
            ...
            if micro_step == grad_accum_steps - 1:
                optimizer_mousse.launch_reduce_scatters()
    ...
    # Get the state dict for quantization and potential pruning
    state_dict_for_quant = base_model.state_dict()

    quant_obj, quant_stats = quantize_state_dict_int8(
        state_dict_for_quant, fp16_embed=args.fp16_embed, quant_bits=args.quant_bits,
    ...
    if master_process:
        ...
        # Pruning loop for quantized model size
        while len(quant_blob) >= limit_bytes:
            log0(f"export: OVERSIZE FALLBACK! Size {len(quant_blob):,} exceeds 16MB limit. Pruning 1% of weights...")
            
            # Prune smallest 1% of matrix parameters in state dict
            with torch.no_grad():
                for n, t in state_dict_for_quant.items():
                    if t.ndim == 2 and t.is_floating_point():
                        # Ensure tensor is on CPU for quantile calculation if it's on GPU
                        t_cpu = t.cpu() if t.is_cuda else t
                        threshold = torch.quantile(t_cpu.abs().flatten().float(), 0.01)
                        t[t.abs() < threshold] = 0.0
            
            # Re-quantize and re-compress
            quant_obj, _qstats = quantize_state_dict_int8(
                state_dict_for_quant, fp16_embed=args.fp16_embed, quant_bits=args.quant_bits,
                quant_bits_mlp=args.quant_bits_mlp, search_clip=args.gptq_lite,
            )
            quant_buf = io.BytesIO()
            ...
            log0(f"export: New compressed size after pruning: {len(quant_blob):,}")
    ...
    if args.ttt_enabled:
        ...
        # LoRA-based TTT: attach lightweight adapters to Q/V projections
        ...
        for i, block in enumerate(base_model.blocks):
            if i < num_frozen:
                continue
            attn = block.attn
            targets: list[CastedLinear] = []
            if isinstance(attn, CausalSelfAttention):
                targets = [attn.c_q, attn.c_v]
            elif isinstance(attn, HymbaAttention):
                targets = [attn.c_q]
            for lin in targets:
                A = torch.randn(lora_rank, lin.in_features, device=device, dtype=torch.float32) * 0.01
                B = torch.zeros(lin.out_features, lora_rank, device=device, dtype=torch.float32)
                A.requires_grad_(True)
                B.requires_grad_(True)
                lin._lora_A = A
                lin._lora_B = B
                lora_params.extend([A, B])
        ttt_opt = torch.optim.AdamW(lora_params, lr=args.ttt_lr, weight_decay=0.01)
        ...
        log0(f"ttt:starting lora_rank={lora_rank} lr={args.ttt_lr} freeze_blocks={num_frozen} epochs={args.ttt_epochs} chunks={total_chunks} lora_params={len(lora_params)}")

        # Pre-Flight Check 2: LoRA TTT Context Switching. 
        # Adapters already hoisted out of the loop. Compile the inner backward step for speed.
        def ttt_step_fn(base_model_x_y):
            ttt_opt.zero_grad()
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                loss = base_model(*base_model_x_y)
            loss.backward()
            ttt_opt.step()
        
        # Don't fullgraph compile the whole loop, just the train step if needed, or leave uncompiled to save 2 min compile overhead.
        # Actually, since it's only a single epoch per chunk, compiling would take longer than the evaluation itself.
        # So using basic Python is faster here!
        ...
        for seq_idx in range(0, total_seqs, args.eval_batch_seqs):
            ...
            # Pre-Flight Check: VRAM Phantom. Clean fragments to prevent OOM
            if (seq_idx // args.eval_batch_seqs) % 10 == 0:
        ...
        for block in base_model.blocks:
            attn = block.attn
            targets_cleanup: list[CastedLinear] = []
            if isinstance(attn, CausalSelfAttention):
                targets_cleanup = [attn.c_q, attn.c_v]
            elif isinstance(attn, HymbaAttention):
                targets_cleanup = [attn.c_q]
            for lin in targets_cleanup:
                lin._lora_A = None
                lin._lora_B = None
        base_model.eval()
